...
class MNISTClassifierNFViT(nn.Module):
    def __init__(self, num_classes, k_probes, epsilon, n_fisher):
        ...
        self.vit = _build_vit_backbone_mnist()
        ...
        v = torch.randn(k_probes, 224 * 224)
...
class MNISTClassifier1F(MNISTClassifierNFViT):
...
def jet_regularizer(model, data):
    mean_v = model.probes.mean(dim=0, keepdim=True).view(1, 1, 224, 224)
...
train_losses_no_jet_between_allf, test_accuracies_no_jet_between_allf = run_experiment(
    ...
    MNISTClassifierBetweenAllF(NUM_CLASSES, K_PROBES, EPSILON),
    train_loader,
    test_loader,
    use_jet=False,
)
...
csv_filename = 'training_results_comparison_vitb16_mnist.csv'
...
plt.tight_layout()
plt.savefig('training_results_comparison_vitb16_mnist.png', dpi=150, bbox_inches='tight')
print("\nComparison plot saved as 'training_results_comparison_vitb16_mnist.png'")
plt.show()
